...
module var
  ...
  real(mytype), save, allocatable, dimension(:,:,:) :: ta3,tb3,tc3,td3,&
       te3,tf3,tg3,th3,ti3,di3,tx3,tz3
...
contains
  ...
  subroutine init_variables
    ...
    call alloc_x(tx1);call alloc_x(tz1)
    ...
    call alloc_y(tx2);call alloc_y(tz2)
    ...
    call alloc_z(tx3); call alloc_z(tz3)
    call alloc_z(di3)
...
!!!!!!!!!!!
    allocate(aamx(ny),bbmx(ny),ccmx(ny),ddmx(ny),eemx(ny),ggmx(ny),hhmx(ny),wwmx(ny),zzmx(ny))
    allocate(rrmx(ny),qqmx(ny),vvmx(ny),ssmx(ny))
    allocate(sssmx(ny),zzzmx(ny),ttmx(ny),uumx(ny))
