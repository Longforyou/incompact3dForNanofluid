...
module variables
  ...
  !!!!!!!
  real(mytype), allocatable,dimension(:) :: aamx,bbmx,ccmx,ddmx,eemx,ggmx,hhmx,wwmx,zzmx !!TIME IMPLICIT, ncl=2
  real(mytype), allocatable,dimension(:) :: rrmx,qqmx,vvmx,ssmx !!TIME IMPLICIT (with HPL), ncl=2
  real(mytype), allocatable,dimension(:) :: sssmx, zzzmx, ttmx, uumx  !!Nona
  real(mytype), allocatable,dimension(:) :: aamxx, bbmxx,ddmxx 
  !!!!!!!
