...
contains
  ...
  subroutine momentum_rhs_eq(dux1,duy1,duz1,rho1,ux1,uy1,uz1,ep1,phi1,divu3)
    ...
    use variables
    use decomp_2d
    use var, only : ta1,tb1,tc1,td1,te1,tf1,tg1,th1,ti1,di1,mu1,mu2,mu3,tx1
    use var, only : rho2,ux2,uy2,uz2,ta2,tb2,tc2,td2,te2,tf2,tg2,th2,ti2,tj2,di2,tx2
    use var, only : rho3,ux3,uy3,uz3,ta3,tb3,tc3,td3,te3,tf3,tg3,th3,ti3,di3,tx3
  ...
  subroutine momentum_full_viscstress_tensor(dux1, duy1, duz1, divu3, mu1)
    ...
    call transpose_x_to_y(td1, tg2)!! dmudx
  ...
  !############################################################################
  !############################################################################
  subroutine scalar_transport_temp(dphi1, rho1, ux1, uy1, uz1, phi1, phi2,schmidt, is_even, is_skew)
    ...
    use var, only : ta1,tb1,tc1,td1,di1,tx1,kapa1,kapa2,kapa3,tg1
    use var, only : rho2,uy2,ta2,tb2,tc2,td2,te2,di2,tx2,tf2,tg2
    use var, only : rho3,uz3,ta3,tb3,tc3,td3,di3,tx3,tf3,tg3
    ...
    real(mytype),intent(in),dimension(xsize(1),xsize(2),xsize(3)) :: phi2
    real(mytype),intent(in),dimension(xsize(1),xsize(2),xsize(3),nrhotime) :: rho1
    ...
    if (evensc) then
      ...
      call derxS (tx1,phi2(:,:,:),di1,sx,ffxpS,fsxpS,fwxpS,xsize(1),xsize(2),xsize(3),1)!*******************************!
      call derxS (tg1,kapa1(:,:,:),di1,sx,ffxpS,fsxpS,fwxpS,xsize(1),xsize(2),xsize(3),1)!*******************************!
    else
    ...
    if (ilmn) then
      ...
      tb1(:,:,:) = ux1(:,:,:) * tb1(:,:,:)-  zoo* tx1(:,:,:) * tb1(:,:,:)-  zoo* tb1(:,:,:)*tb1(:,:,:)*phi2(:,:,:)-xalpha*tg1(:,:,:)*tb1(:,:,:)!少两个系数!
    endif
    ...
    ta1(:,:,:) = kapa1(:,:,:)*xalpha*ta1(:,:,:)- tb1(:,:,:)!*******************************!

    call transpose_x_to_y(phi1(:,:,:),td2(:,:,:))
    call transpose_x_to_y(phi2(:,:,:),tf2(:,:,:))!*******************************!
    call transpose_x_to_y(kapa1,kapa2)!*******************************!
    !Y PENCILS

    if (evensc) then
      call deryS (tc2,td2(:,:,:),di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
      call deryS (tx2,tf2(:,:,:),di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)!*******************************!
      call deryyS (ta2,td2(:,:,:),di2,sy,sfypS,ssypS,swypS,ysize(1),ysize(2),ysize(3),1)
      call deryS (tg2,kapa2(:,:,:),di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)!*******************************!
    else
    ...
    if (ilmn) then
       ...
       tb2(:,:,:) = uy2(:,:,:) * tc2(:,:,:)- zoo*tx2(:,:,:)*tc2(:,:,:)-zoo*tc2(:,:,:)*tc2(:,:,:)*tf2(:,:,:)-xalpha*tg2(:,:,:)*tc2(:,:,:)!少两个系数!
    endif


    ! Add convective and diffusive scalar terms of y-pencil
    tc2(:,:,:) = kapa2(:,:,:)*xalpha*ta2(:,:,:) - tb2(:,:,:)
    call transpose_y_to_z(td2(:,:,:),td3(:,:,:))
    call transpose_y_to_z(tf2(:,:,:),tf3(:,:,:))!*******************************!
    call transpose_x_to_y(kapa2,kapa3)!*******************************!

    !Z PENCILS

    if (evensc) then
      call derzS (tb3,td3(:,:,:),di3,sz,ffzpS,fszpS,fwzpS,zsize(1),zsize(2),zsize(3),1)
      call derzS (tx3,tf3(:,:,:),di3,sz,ffzpS,fszpS,fwzpS,zsize(1),zsize(2),zsize(3),1)!*******************************!
      call derzS (tg3,kapa3(:,:,:),di3,sz,ffzpS,fszpS,fwzpS,zsize(1),zsize(2),zsize(3),1)
    else
    ...
    if (ilmn) then
      ...
      tb3(:,:,:) = uz3(:,:,:) * tb3(:,:,:)- zoo*tx3(:,:,:)*tb3(:,:,:)-zoo*tb3(:,:,:)*tb3(:,:,:)*tf3(:,:,:)-xalpha*tg3(:,:,:)*tb3(:,:,:)!!少两个系数!
    endif
  ...
  endsubroutine scalar_transport_temp
  !############################################################################
  !############################################################################
  subroutine scalar_transport_nano(dphi1, rho1, ux1, uy1, uz1, phi1, phi2,schmidt, is_even, is_skew)
   ...
   use var, only : ta1,tb1,tc1,td1,di1,tx1
   use var, only : rho2,uy2,ta2,tb2,tc2,td2,te2,di2,tx2,tf2
   use var, only : rho3,uz3,ta3,tb3,tc3,td3,di3,tx3,tf3
   ...
   if (evensc) then
     call derxxS (ta1,phi1(:,:,:),di1,sx,sfxpS,ssxpS,swxpS,xsize(1),xsize(2),xsize(3),1)
     call derxxS (tx1,phi2(:,:,:),di1,sx,sfxpS,ssxpS,swxpS,xsize(1),xsize(2),xsize(3),1)!*************************************
   else
     call derxxS (ta1,phi1(:,:,:),di1,sx,sfxS,ssxS,swxS,xsize(1),xsize(2),xsize(3),0)
   endif

   ! Add convective and diffusive scalar terms of x-pencil
   ta1(:,:,:) = xalpha*ta1(:,:,:) +          tx1(:,:,:)- tb1(:,:,:)!少系数
   ...
   if (evensc) then
     call deryS (tc2,td2(:,:,:),di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
     call deryS (tx1,tf2(:,:,:),di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)!*************************************
     if (skewsc) call deryS (te2,tb2,di2,sy,ffyS,fsyS,fwyS,ppy,ysize(1),ysize(2),ysize(3),0)
     call deryyS (ta2,td2(:,:,:),di2,sy,sfypS,ssypS,swypS,ysize(1),ysize(2),ysize(3),1)
     call deryyS (tx2,tf2(:,:,:),di2,sy,sfypS,ssypS,swypS,ysize(1),ysize(2),ysize(3),1)!*************************************
   else
     call deryS (tc2,td2(:,:,:),di2,sy,ffyS,fsyS,fwyS,ppy,ysize(1),ysize(2),ysize(3),0)
   ...
   if (istret.ne.0) then
      ...
      do k = 1,ysize(3)
         do j = 1,ysize(2)
            do i = 1,ysize(1)
               ta2(i,j,k) = ta2(i,j,k)*pp2y(j)-pp4y(j)*tc2(i,j,k)
               tx2(i,j,k) = tx2(i,j,k)*pp2y(j)-pp4y(j)*tx1(i,j,k)!*************************************
            enddo
         enddo
      enddo
   endif
   ...
   if (evensc) then
     call derzzS (ta3,td3(:,:,:),di3,sz,sfzpS,sszpS,swzpS,zsize(1),zsize(2),zsize(3),1)
     call derzzS (tx3,tf3(:,:,:),di3,sz,sfzpS,sszpS,swzpS,zsize(1),zsize(2),zsize(3),1)!*************************************
   else
     call derzzS (ta3,td3(:,:,:),di3,sz,sfzS,sszS,swzS,zsize(1),zsize(2),zsize(3),0)
   endif

   ! Add convective and diffusive scalar terms of z-pencil
   ta3(:,:,:) = xalpha*ta3(:,:,:) +          xalpha*tx3(:,:,:)- tb3(:,:,:)!*************************************
 ...
 endsubroutine scalar_transport_nano
  !###########################################################################
  !############################################################################
  subroutine scalar(dphi1, rho1, ux1, uy1, uz1, phi1)
   ...
   call scalar_transport_temp(dphi1(:,:,:,:,1), rho1, ux1, uy1, uz1, phi1(:,:,:,1), phi1(:,:,:,2),sc(1), is_even=sc_even(1), is_skew=sc_skew(1))
   call scalar_transport_nano(dphi1(:,:,:,:,2), rho1, ux1, uy1, uz1, phi1(:,:,:,2), phi1(:,:,:,1),sc(2), is_even=sc_even(2), is_skew=sc_skew(2))
   call scalar_settling(dphi1, phi1(:,:,:,2), 2)
          ! If LES modelling is enabled, add the SGS stresses
   if (ilesmod.ne.0.and.jles.le.2.and.jles.gt.0) then
         dphi1(:,:,:,1, 1) = dphi1(:,:,:,1,1) + sgsphi1(:,:,:,1)
   endif
