...
module tbl
  ...
  PUBLIC :: init_tbl, boundary_conditions_tbl, postprocess_tbl, tbl_flrt,set_fluid_properties_tbl
...
contains
  ...
  subroutine init_tbl (ux1,uy1,uz1,ep1,phi1)
    ...
    call set_fluid_properties_tbl(mu1,phi1)
...
994 format('vort',I3.3)
  ...
  subroutine set_fluid_properties_tbl( mu1,phi1)
   ...
   do k=1,xsize(3)
      do j=1,xsize(2)
         do i=1,xsize(1)
            mu1(i,j,k)=exp(0.4*(0.25/phi1(i,j,k,1)-one))
            mu1(i,j,k)=1-0.1*phi1(i,j,k,1)
            if(phi1(i,j,k,1).ge.1) then
               mu1(i,j,k)=0.8
            elseif(phi1(i,j,k,1).le.0.25)then
               mu1(i,j,k)=0.95
            endif
 ...
 endsubroutine set_fluid_properties_tbl
