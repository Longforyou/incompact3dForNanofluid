...
module ludecomp
  ...
  private
  public :: ludecomp7, ludecomp9,ludecompx
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  interface ludecompx
     module procedure ludecomp9_12x 
  end interface ludecompx
contains
...
!Decomposition LU matrix nonadiag
  subroutine ludecomp9_12x(aamx,bbmx,ccmx,ddmx,eemx,qqmx,ggmx,hhmx,ssmx,rrmx,vvmx,wwmx,zzmx,ttmx,uumx,sssmx,zzzmx,ny)
   !
   !*******************************************************************
       use decomp_2d, only : mytype
       use param
       ...
       integer :: i,j,k,ny
       real(mytype),dimension(ny), intent(in)  :: aamx,bbmx,ccmx,ddmx,eemx,rrmx,qqmx,ttmx,uumx
       real(mytype),dimension(ny), intent(out) :: ggmx,hhmx,ssmx,sssmx
       real(mytype),dimension(ny), intent(out) :: vvmx,wwmx,zzmx,zzzmx
   
       real(mytype), dimension(ny,ny) :: mata, matl, matu, prod
   
       ggmx=zero;hhmx=zero;ssmx=zero;sssmx=zero
       vvmx=zero;wwmx=zero;zzmx=zero;zzzmx=zero
       ...
       zzzmx(2)=ddmx(2)/ggmx(1)
       ggmx(2)=aamx(2)-zzzmx(2)*hhmx(1)
       hhmx(2)=bbmx(2)-zzzmx(2)*ssmx(1)
       ssmx(2)=ccmx(2)-zzzmx(2)*sssmx(1)
       sssmx(2)=rrmx(2)-zzzmx(2)*ttmx(1)
   
       zzmx(3)=eemx(3)/ggmx(1)
       zzzmx(3)=(ddmx(3)-zzmx(3)*hhmx(1))/ggmx(2)
       ggmx(3)=aamx(3)-zzmx(3)*ssmx(1)-zzzmx(3)*hhmx(2)
       hhmx(3)=bbmx(3)-zzmx(3)*sssmx(1)-zzzmx(3)*ssmx(2)
       ssmx(3)=ccmx(3)-zzmx(3)*ttmx(1)-zzzmx(3)*sssmx(2)
       sssmx(3)=rrmx(3)-zzzmx(3)*ttmx(2)
   
       wwmx(4)=qqmx(4)/ggmx(1)
       zzmx(4)=(eemx(4)-wwmx(4)*hhmx(1))/ggmx(2)
       zzzmx(4)=(ddmx(4)-zzmx(4)*hhmx(2)-wwmx(4)*ssmx(1))/ggmx(3)
   
       ggmx(4)=aamx(4)-wwmx(4)*sssmx(1)-zzmx(4)*ssmx(2)-zzzmx(4)*hhmx(3)
       hhmx(4)=bbmx(4)-wwmx(4)*ttmx(1)-zzmx(4)*sssmx(2)-zzzmx(4)*ssmx(3)
       ssmx(4)=ccmx(4)-zzmx(4)*ttmx(2)-zzzmx(4)*sssmx(3)
       sssmx(4)=rrmx(4)-zzzmx(4)*ttmx(3)
   
       do j=5,ny
          vvmx(j)= uumx(j)/ggmx(j-4)
          wwmx(j)=(qqmx(j)-vvmx(j)*hhmx(j-4))/ggmx(j-3)
          zzmx(j)=(eemx(j)-wwmx(j)*hhmx(j-3)-vvmx(j)*ssmx(j-4))/ggmx(j-2)
          zzzmx(j)=(ddmx(j)-zzmx(j)*hhmx(j-2)-wwmx(j)*ssmx(j-3)-vvmx(j)*sssmx(j-4))/ggmx(j-1)
   
          ggmx(j)=aamx(j)-vvmx(j)*ttmx(j-4)-wwmx(j)*sssmx(j-3)-zzmx(j)*ssmx(j-2)-zzzmx(j)*hhmx(j-1)
          hhmx(j)=bbmx(j)-wwmx(j)*ttmx(j-3)-zzmx(j)*sssmx(j-2)-zzzmx(j)*ssmx(j-1)
          ssmx(j)=ccmx(j)-zzmx(j)*ttmx(j-2)-zzzmx(j)*sssmx(j-1)
          sssmx(j)=rrmx(j)-zzzmx(j)*ttmx(j-1)
   
       enddo
     end subroutine ludecomp9_12x
!*******************************************************************
...
module matinv
  ...
  private
  public :: septinv, nonainv,nonainvx,nonainvy
  !
  ...
  interface nonainvx
     module procedure nonainv_12x
  end interface nonainvx
  interface nonainvy
  module procedure nonainv_12y
end interface nonainvy
contains
...
!INVERSE NONADIAG
  ...
  subroutine nonainv_12x(xSol,bbb,nx,ny,nz,aamxx,bbmxx,ccm,ddmxx,rrm,ttm)
!
!********************************************************************
    use decomp_2d
    USE var, ONLY: mu1,mu2,mu3
    use param
    USE derivY
    use variables, ONLY:pp2y,aamx,bbmx,ccmx,ddmx,eemx,qqmx,uumx,rrmx,ttmx,vvmx,wwmx,zzmx,zzzmx,ggmx,hhmx,ssmx,sssmx
    use ludecomp
    
    implicit none

    integer :: i,j,k,nx,ny,nz,jf
    real(mytype),dimension(nx,ny,nz),intent(out) :: xsol
    real(mytype),dimension(nx,ny,nz),intent(in) :: bbb
    real(mytype),dimension(ny),intent(in)    ::  aamxx,bbmxx,ccm,ddmxx,rrm,ttm
    ...
    do k=1,nz
       do i=1,nx
         ...
         aamx(1)=one
         aamx(ny)=one
         bbmx(2     )=bbmxx(2     )*mu2(i,3,k)+alsa2y/pp2y(3)
         bbmx(ny-1  )=bbmxx(ny-1  )*mu2(i,ny,k)+alsamy/pp2y(ny)
         bbmx(3     )=bbmxx(3     )*mu2(i,4,k)+alsa3y/pp2y(4)
         bbmx(ny-2  )=bbmxx(ny-2  )*mu2(i,ny-1,k)+alsaty/pp2y(ny-1)
         bbmx(4     )=bbmxx(4     )*mu2(i,5,k)+alsa4y/pp2y(5)
         bbmx(ny-3  )=bbmxx(ny-3  )*mu2(i,ny-2,k)+alsatty/pp2y(ny-2)
         do jf=5:ny-4
            bbmx(jf)=bbmxx(jf)*mu2(i,jf+1,k)+alsajy/pp2y(jf+1)
         enddo
         bbmx(1 )=zero
         bbmx(ny)=zero
         do jf=2:ny-2
            ccmx(jf)=mu2(i,jf+2,k)*ccm(jf)
         enddo
         ccmx(1)=zero
         ccmx(ny)=zero
         do jf=2:ny-3
            rrmx(jf)=mu2(i,jf+3,k)*rrm(jf)
         enddo
         rrmx(1)=zero
         rrmx(ny)=zero
         do jf=2:ny-4
            ttmx(jf)=mu2(i,jf+4,k)*ttm(jf)
         enddo
         ttmx(1)=zero
         ttmx(ny)=zero

         ddmx(2     )=ddmxx(2     )*mu2(i,1,k)+alsa2y/pp2y(1)
         ddmx(ny-1  )=ddmxx(ny-1  )*mu2(i,ny-2,k)+alsamy/pp2y(ny-2)
         ddmx(3     )=ddmxx(3     )*mu2(i,2,k)+alsa3y/pp2y(2)
         ddmx(ny-2  )=ddmxx(ny-2  )*mu2(i,ny-3,k)+alsaty/pp2y(ny-3)
         ddmx(4     )=ddmxx(4     )*mu2(i,3,k)+alsa4y/pp2y(3)
         ddmx(ny-3  )=ddmxx(ny-3  )*mu2(i,ny-4,k)+alsatty/pp2y(ny-4)
         do jf=5:ny-4
            ddmx(jf)=ddmxx(jf)*mu2(i,jf-1,k)+alsajy/pp2y(jf-1)
         enddo
         ddmx(1 )=zero
         ddmx(ny)=zero
         eemx=ccm
         do jf=3:ny-1
            eemx(jf)=ccm(jf)*mu2(i,jf-2,k)
         enddo
         qqmx=rrm
         do jf=4:ny-1
            qqmx(jf)=rrm(jf)*mu2(i,jf-3,k)
         enddo
         uumx=ttm
         do jf=5:ny-1
            uumx(jf)=ttm(jf)*mu2(i,jf-4,k)
         enddo
         call ludecompx(aamx,bbmx,ccmx,ddmx,eemx,qqmx,ggmx,hhmx,ssmx,rrmx,vvmx,wwmx,zzmx,ttmx,uumx,sssmx,zzzmx,ny)
          !going down
          xSol(i,1,k)=bbb(i,1,k);
          xSol(i,2,k)=bbb(i,2,k)-zzzmx(2)*xSol(i,1,k);
          xSol(i,3,k)=bbb(i,3,k)-zzmx(3)*xSol(i,1,k)-zzzmx(3)*xSol(i,2,k);
          xSol(i,4,k)=bbb(i,4,k)-wwmx(4)*xSol(i,1,k)-zzmx(4)*xSol(i,2,k)-zzzmx(4)*xSol(i,3,k);

          do j=5,ny
             xSol(i,j,k)=bbb(i,j,k)-vvmx(j)*xSol(i,j-4,k)-wwmx(j)*xSol(i,j-3,k) &
                  -zzmx(j)*xSol(i,j-2,k)-zzzmx(j)*xSol(i,j-1,k);
          enddo
          !

          !going up
          xSol(i,ny,k)=xSol(i,ny,k)/ggmx(ny);
          xSol(i,ny-1,k)=(xSol(i,ny-1,k)-hhmx(ny-1)*xSol(i,ny,k))/ggmx(ny-1);
          xSol(i,ny-2,k)=(xSol(i,ny-2,k)-hhmx(ny-2)*xSol(i,ny-1,k)-ssmx(ny-2)*xSol(i,ny,k))/ggmx(ny-2);
          xSol(i,ny-3,k)=(xSol(i,ny-3,k)-hhmx(ny-3)*xSol(i,ny-2,k)-ssmx(ny-3)*xSol(i,ny-1,k) &
               -sssmx(ny-3)*xSol(i,ny,k))/ggmx(ny-3);

          do j=ny-4,1,-1
             xSol(i,j,k)=(xSol(i,j,k)-hhmx(j)*xSol(i,j+1,k)-ssmx(j)*xSol(i,j+2,k) &
                  -sssmx(j)*xSol(i,j+3,k)-ttmx(j)*xSol(i,j+4,k))/ggmx(j);
          enddo

       enddo
    enddo

  end subroutine nonainv_12x
  subroutine nonainv_12y(xSol,bbb,nx,ny,nz,aamxx,bbmxx,ccm,ddmxx,rrm,ttm,phi2)
   !
   !********************************************************************
       use decomp_2d
       USE var, ONLY: mu1,mu2,mu3,phi1
       use param
       USE derivY
       use variables, ONLY:pp2y,aamx,bbmx,ccmx,ddmx,eemx,qqmx,uumx,rrmx,ttmx,vvmx,wwmx,zzmx,zzzmx,ggmx,hhmx,ssmx,sssmx
       use ludecomp
       ...
       integer :: i,j,k,nx,ny,nz,jf
       real(mytype),dimension(nx,ny,nz),intent(out) :: xsol
       real(mytype),dimension(nx,ny,nz),intent(in) :: bbb,phi2
       real(mytype),dimension(ny),intent(in)    ::  aamxx,bbmxx,ccm,ddmxx,rrm,ttm
       ...
       do k=1,nz
          do i=1,nx
            ...
            aamx(:)=(phi2(i,:,k)+1)*aamxx(:)/sc(2)/500
            aamx=one/pp2y+aamx
            aamx(1)=one
            aamx(ny)=one
            bbmx(2     )=bbmxx(2     )*mu2(i,3,k)+alsa2y/pp2y(3)
            bbmx(2     )=bbmxx(2     )*(phi2(i,3,k)+1))/sc(2)/500+alsa2y/pp2y(3)
            bbmx(ny-1  )=bbmxx(ny-1  )*(phi2(i,ny,k)+1))/sc(2)/500+alsamy/pp2y(ny)
            bbmx(3     )=bbmxx(3     )*(phi2(i,4,k)+1))/sc(2)/500+alsa3y/pp2y(4)
            bbmx(ny-2  )=bbmxx(ny-2  )*(phi2(i,ny-1,k)+1))/sc(2)/500+alsaty/pp2y(ny-1)
            bbmx(4     )=bbmxx(4     )*(phi2(i,5,k)+1))/sc(2)/500+alsa4y/pp2y(5)
            bbmx(ny-3  )=bbmxx(ny-3  )*(phi2(i,ny-2,k)+1))/sc(2)/500+alsatty/pp2y(ny-2)
            do jf=5:ny-4
               bbmx(jf)=bbmxx(jf)*(phi2(i,jf+1,k)+1))/sc(2)/500+alsajy/pp2y(jf+1)
            enddo
            bbmx(1 )=zero
            bbmx(ny)=zero
            do jf=2:ny-2
               ccmx(jf)=(phi2(i,jf+2,k)+1))/sc(2)/500*ccm(jf)
            enddo
            ccmx(1)=zero
            ccmx(ny)=zero
            do jf=2:ny-3
               rrmx(jf)=(phi2(i,jf+3,k)+1))/sc(2)/500*rrm(jf)
            enddo
            rrmx(1)=zero
            rrmx(ny)=zero
            do jf=2:ny-4
               ttmx(jf)=(phi2(i,jf+4,k)+1))/sc(2)/500*ttm(jf)
            enddo
            ttmx(1)=zero
            ttmx(ny)=zero
   
            ddmx(2     )=ddmxx(2     )*(phi2(i,1,k)+1))/sc(2)/500+alsa2y/pp2y(1)
            ddmx(ny-1  )=ddmxx(ny-1  )*(phi2(i,n-2,k)+1))/sc(2)/500+alsamy/pp2y(ny-2)
            ddmx(3     )=ddmxx(3     )*(phi2(i,2,k)+1))/sc(2)/500+alsa3y/pp2y(2)
            ddmx(ny-2  )=ddmxx(ny-2  )*(phi2(i,n-3,k)+1))/sc(2)/500+alsaty/pp2y(ny-3)
            ddmx(4     )=ddmxx(4     )*(phi2(i,3,k)+1))/sc(2)/500+alsa4y/pp2y(3)
            ddmx(ny-3  )=ddmxx(ny-3  )*(phi2(i,n-4,k)+1))/sc(2)/500+alsatty/pp2y(ny-4)
            do jf=5:ny-4
               ddmx(jf)=ddmxx(jf)*(phi2(i,jf-1,k)+1))/sc(2)/500+alsajy/pp2y(jf-1)
            enddo
            ddmx(1 )=zero
            ddmx(ny)=zero
            eemx=ccm
            do jf=3:ny-1
               eemx(jf)=ccm(jf)*(phi2(i,jf-2,k)+1))/sc(2)/500
            enddo
            qqmx=rrm
            do jf=4:ny-1
               qqmx(jf)=rrm(jf)*(phi2(i,jf-3,k)+1))/sc(2)/500
            enddo
            uumx=ttm
            do jf=5:ny-1
               uumx(jf)=ttm(jf)*(phi2(i,jf-4,k)+1))/sc(2)/500
            enddo
            call ludecompx(aamx,bbmx,ccmx,ddmx,eemx,qqmx,ggmx,hhmx,ssmx,rrmx,vvmx,wwmx,zzmx,ttmx,uumx,sssmx,zzzmx,ny)
             !going down
             xSol(i,1,k)=bbb(i,1,k);
...
!        call multmatrix7(te2,tb2,uy2,0) !NEED TO BE DONE: ADJUST PARITY WHEN TB2=UY, NPAIRE in parameters 
  ...
  if (isecondder.eq.5) then
     ...
     elseif ((ncly1.eq.1).and.(nclyn.eq.1)) then
        !NOT READY
     elseif ((ncly1.eq.2).and.(nclyn.eq.2)) then
        call multmatrix9(td2,ta2,tb2,1)
     endif
  endif
  ...
  if (isecondder.eq.5) then
     ...
     elseif ((ncly1.eq.2).and.(nclyn.eq.2)) then
      ...
      call nonainvx(tb2,ta2,ysize(1),ysize(2),ysize(3),aamxx,bbmxx,ccm,ddmxx,rrm,ttm)
...
subroutine multmatrix9(td2,ta2,ux2,iwhen)
  !
  !********************************************************************
  USE param
  USE variables
  ...
  integer :: i,j,k,code,iwhen
  !real(mytype) :: xcst ! modification module param
  real(mytype),dimension(ysize(1),ysize(2),ysize(3)), intent(inout) :: ux2
  real(mytype),dimension(ysize(1),ysize(2),ysize(3)), intent(inout) :: td2,ta2
  real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: di2,phi2
  ...
  elseif (ncly1.eq.2) then
     ...
     elseif(iwhen.eq.3) then
      td2(:,:,:)=(phi2(:,:,:,1)+1)*td2(:,:,:)/sc(2)/500  !500=deltaT
     endif
...
!************************************************************
!
subroutine scalarimp(ux1,uy1,uz1,phi1,phi11,dphi1,is)
  !
  !************************************************************
  ...
  USE param
  USE variables
  USE var, ONLY: di1,tg1,th1,ti1,td1,ta2,tb2,tc2,td2,di2,di3,ta3,tb3,tf1,te2,tc3,tx1,tx2,tx3,tz1,tz2,tz3
  USE decomp_2d
  USE derivY
  USE MPI
  use matinv
  ...
  real(mytype),dimension(xsize(1),xsize(2),xsize(3)) :: phi1,phi11
  real(mytype),dimension(xsize(1),xsize(2),xsize(3),ntime) :: dphi1
  integer :: is

  real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: uy2,uz2,phi2,phi22
  real(mytype),dimension(zsize(1),zsize(2),zsize(3)) :: uz3,phi3,phi33
  real(mytype),dimension(ysize(1),ysize(3)) :: bc1
  
  integer :: ijk,nvect1,nvect2,nvect3,i,j,k,nxyz,code

  !parametres CL
  real(mytype) :: x,y,z,r,lambda,phislbda,adiab,tjet,liss,Tlow,deltaT,Db0,Dt0,Leb,Let
  Tlow=500.
  deltaT=500.
  Leb=1.e+7
  Let=500
  ...
  ta3=zero;tb3=zero;tf1=zero;te2=zero
  tc3=zero;tx1=zero;tx2=zero;tx3=zero
  tz1=zero;tz2=zero;tz3=zero
  ...
  !X PENCILS
  !tg1=ux1*phi1
  call derxS (th1,phi1,di1,sx,ffxpS,fsxpS,fwxpS,xsize(1),xsize(2),xsize(3),1) ! npaire=0 pour phi
  if(is.eq.1)then
      call derxS (tf1,phi11,di1,sx,ffxpS,fsxpS,fwxpS,xsize(1),xsize(2),xsize(3),1)
      tx1(:,:,:)=th1(:,:,:)*th1(:,:,:)*phi11(:,:,:)!\theta*\theta
      tf1(:,:,:)=tf1(:,:,:)*th1(:,:,:)!\theta*c
  endif
  if(is.eq.2)then
   call derxS (tf1,phi11,di1,sx,ffxpS,fsxpS,fwxpS,xsize(1),xsize(2),xsize(3),1)
   tx1(:,:,:)=tf1(:,:,:)/(phi11(:,:,:)+Tlow/deltaT)
   call derxS (tz1,tx1,di1,sx,ffxpS,fsxpS,fwxpS,xsize(1),xsize(2),xsize(3),1)
   tz1(:,:,:)=tz1(:,:,:)*phi1(:,:,:)
   tf1(:,:,:)=tf1(:,:,:)*th1(:,:,:)!\theta*c
  endif
  th1=ux1*th1!\u*\theta
  ...
  call deryS (tb2,phi2,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)! npaire=0 pour phi
  if(is.eq.1)then
   call deryS (te2,phi22,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
   tx2(:,:,:)=tb2(:,:,:)*tb2(:,:,:)*phi22(:,;,:)!\theta*\theta
   te2(:,:,:)=te2(:,:,:)*tb2(:,:,:)!\theta*c
  endif
  if(is.eq.2)then
   call deryS (te2,phi22,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
   tx2(:,:,:)=te2(:,:,:)/(phi22(:,:,:)+Tlow/deltaT)
   call deryS (tz2,tx2,di2,sy,ffypS,fsypS,fwypS,ppy,ysize(1),ysize(2),ysize(3),1)
   tz2(:,:,:)=tz2(:,:,:)*phi2(:,:,:)
   te2(:,:,:)=te2(:,:,:)*tb2(:,:,:)
  endif
  tb2=uy2*tb2!\u*\theta
  if (istret.ne.0) then
     ...
     enddo
   if(is.eq.2)then
      ta2(:,:,:)=(phi22(:,:,:)+Tlow/deltaT)*ta2(:,:,:)
   endif

  endif

  call transpose_y_to_z(phi2,phi3)
  call transpose_y_to_z(phi22,phi33)
  ...
  if(is.eq.1)then
   call derzS (tc3,phi33,di3,sz,ffzpS,fszpS,fwzpS,zsize(1),zsize(2),zsize(3),1)
   tx3(:,:,:)=tb3(:,:,:)*tb3(:,:,:)*phi33(:,:,:)
   tc3(:,:,:)=tc3(:,:,:)*tb3(:,:,:)
  endif
  if(is.eq.2)then
   call derzS (tc3,phi33,di3,sz,ffzpS,fszpS,fwzpS,zsize(1),zsize(2),zsize(3),1)
   tx3(:,:,:)=tc3(:,:,:)/(phi33(:,:,:)+Tlow/deltaT)
   call derzS (tz3,tx3,di3,sz,ffzpS,fszpS,fwzpS,zsize(1),zsize(2),zsize(3),1)
   tz3(:,:,:)=tz3(:,:,:)*phi3(:,:,:)
   tc3(:,:,:)=tc3(:,:,:)*tb3(:,:,:)
  endif
  ...
  tb3=uz3*tb3
  if(is.eq.2)then
   ta3(:,:,:)=(phi33(:,:,:)+Tlow/deltaT)*ta3(:,:,:)
  endif
  do k=1,zsize(3)
   do j=1,zsize(2)
      do i=1,zsize(1)
         if(is.eq.1)then
         tb3(i,j,k)=tb3(i,j,k)-xnu/sc(1)/Leb*(phi3(i,j,k)*deltaT+Tlow )*tc3(i,j,k)-xnu/sc(1)/Let*tx3(i,j,k)/(phi3(i,j,k)+Tlow/deltaT )
         endif
         if(is.eq.2)then
         tb3(i,j,k)=tb3(i,j,k)-xnu/sc(2)*(1./deltaT+Leb/Let/(phi33(i,j,k)+Tlow/deltaT))*tc3(i,j,k)-xnu*Leb/sc(2)/Let*tz3(i,j,k)
        endif
      enddo
   enddo
enddo
  ...
  do k=1,ysize(3)
     do j=1,ysize(2)
        do i=1,ysize(1)
           ...
           if(is.eq.1)then
           td2(i,j,k)=td2(i,j,k)+tb2(i,j,k)-xnu/sc(1)/Leb*(phi2(i,j,k)*deltaT+Tlow )*te2(i,j,k)-xnu/sc(1)/Let*tx2(i,j,k)/(phi2(i,j,k)+Tlow/deltaT)
           !FIRST DERIVATIVE
           endif
           if(is.eq.2)then
           td2(i,j,k)=td2(i,j,k)+tb2(i,j,k)-xnu/sc(2)*(1./deltaT+Leb/Let/(phi22(i,j,k)+Tlow/deltaT))*te2(i,j,k)-xnu*Leb/sc(2)/Let*tz2(i,j,k)
           endif
  ...
  do k=1,xsize(3)
     do j=1,xsize(2)
        do i=1,xsize(1)
           ...
           if(is.eq.1)then
           th1(i,j,k)= td1(i,j,k)+th1(i,j,k)-xnu/sc(1)/Leb*(phi1(i,j,k)*deltaT+Tlow)*tf1(i,j,k)-xnu/sc(1)/Let*tx1(i,j,k)/(phi1(i,j,k)+Tlow/deltaT) 
           !FIRST DERIVATIVE
           endif
           if(is.eq.2)then
            th1(i,j,k)=td1(i,j,k)+th1(i,j,k)-xnu/sc(2)*(1./deltaT+Leb/Let/(phi33(i,j,k)+Tlow/deltaT))*tf1(i,j,k)-xnu*Leb/sc(2)/Let*tz1(i,j,k)
            endif
        enddo
     enddo
  enddo
  ...
  do k=1,xsize(3)
     do j=1,xsize(2)
        do i=1,xsize(1)
         if(is.eq.1)then
           tg1(i,j,k)= (xnu/sc(1))*tg1(i,j,k)-th1(i,j,k)
         endif
         if(is.eq.2)then
            tg1(i,j,k)= (xnu/sc(2)/deltaT)*tg1(i,j,k)-th1(i,j,k)
          endif
        enddo
     enddo
  enddo
...
!!!!!!!!!!!!!!!!!!!!
  ...
  if (isecondder.ne.5) then
     ...
     call multmatrix9(td2,ta2,phi2,2)
  endif
  !right hand side
  ta2(:,:,:) = ta2(:,:,:) + td2(:,:,:)
  ...
  if (isecondder.ne.5) then
    ...
    if (isecondder.eq.5) then
     ...
     elseif ((ncly1.eq.2).and.(nclyn.eq.2)) then
      ...
      if(is.eq.2)then
         call nonainvy(phi2,ta2,ysize(1),ysize(2),ysize(3),aamxx,bbmxx,ccm,ddmxx,rrm,ttm,phi22)
      endif
     endif
  endif
